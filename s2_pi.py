...
# Defining ready and retract angular configurations + variable to store feedback angular positions
readyPoint = PyJaco2API.TrajectoryPoint()
destPoint = PyJaco2API.TrajectoryPoint()
retractPoint = PyJaco2API.TrajectoryPoint()
feedbackPos = PyJaco2API.AngularPosition()

readyPoint.Position.HandMode = PyJaco2API.HAND_MODE.HAND_NOMOVEMENT
readyPoint.Position.Type = PyJaco2API.POSITION_TYPE.ANGULAR_POSITION
readyPoint.LimitationsActive = 1
readyPoint.Limitations.speedParameter1 = 20.0
readyPoint.Limitations.speedParameter2 = 20.0
readyPoint.Position.Actuators.Actuator1 = 320.0
readyPoint.Position.Actuators.Actuator2 = 194.84
readyPoint.Position.Actuators.Actuator3 = 113.25
readyPoint.Position.Actuators.Actuator4 = 250.57
readyPoint.Position.Actuators.Actuator5 = 124.08
readyPoint.Position.Actuators.Actuator6 = 29.18
...
#            self.pi.callback(pin, pigpio.EITHER_EDGE, self.input_callback)
        # when a user wishes to set the state of a digital output pin
        elif client_cmd == 'connectToArm':
            print('ConnectToArm command initiated')
            returnValue = PyJaco2API.API_init()                         
            print('Arm connected')
            ...
            destPoint.LimitationsActive = 1
            destPoint.Limitations.speedParameter1 = 20.0
            destPoint.Limitations.speedParameter2 = 20.0
            
            print("test")
            
            print("angle1:",float(payload['angle1']))
            
            destPoint.Position.Actuators.Actuator1 = float(payload['angle1'])
            destPoint.Position.Actuators.Actuator2 = float(payload['angle2'])
            destPoint.Position.Actuators.Actuator3 = float(payload['angle3'])
            destPoint.Position.Actuators.Actuator4 = float(payload['angle4'])
            destPoint.Position.Actuators.Actuator5 = float(payload['angle5'])
            destPoint.Position.Actuators.Actuator6 = float(payload['angle6']) 

            print(destPoint)
            
            PyJaco2API.API_sendAdvanceTraj(destPoint)  
